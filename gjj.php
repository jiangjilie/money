	...
	<head>
			              ...
			              <fieldset>
			                  <div class="form-group" >
			                      <div  style="float: right;">
									...
									<a href="javascript:;" id="import" title="点击导入记录" onclick="inportExcel()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-import" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">导入公积金</span><span class="l-btn-icon icon-import">&nbsp;</span></span></a>
		...
		<thead>
		  <tr style="background-color:#438eb9;color: #FFF;">
			...
			<th class="tr-th" style="text-align:center;padding:10px">单位公积金</th>
			<th class="tr-th" style="text-align:center;padding:10px">个人公积金</th>
		...
		<script>
			...
			$("#add").click(function(){
				...
				layer.open({
					 ...
					 closeBtn: 1,
					 anim: 2,
					 content: "gjj_luru.php?user="+user,
					  // btn: ["", "取消"],
		...
		$("#button").click(function(){
					...
					success:function(msg){
									...
									var   data = eval("("+msg+")"); //将返回的json数据进解析，并赋给data									
										...
										function excel_select(ids) {
											window.location.href= "excel_gjj_select.php?id=" + ids ;
													}
									...
									var nd = $("#yf").val();
									 if(dept=="" &&  name=="" && nd==""&& yf=="" ){
											layer.open({
											 ...
											 offset:'t',  //设置弹出位置
											 shadeClose: true,
											 content: 'delete_gjj.php?dept=' + dept + '&name=' + name + '&nd=' + nd+ '&yf=' + yf ,
											 ...
											 yes: function (index, layero) {
												 var thisIndex = index;
												  ...
												  layer.confirm('<h4 style="color:red;padding:5px5px;magin:2px;">警告：真的确定清空表中所有数据吗</h4></center>', { icon: 5, title: '提示' }, function (index) {
														window.location.href= "delete_gjj_php.php?dept=" + dept + "&name=" + name + "&phone=" + phone ;										                                									                 
																																			});
																			},//layer嵌套  *****重要
														});
														}else{
			...
			<script>
				...
				function excel(){				
					...
					// window.open(url, "_blank");
					window.location.href= "excel_gjj.php?dept=" + dept + "&name=" + name + "&nd=" + nd+ "&yf=" + yf+"&company="+company;
								}
			...
			function inportExcel(){		
					layer.open({
					 type: 2,
					 title:'<center><h3> 公积金数据导入</center>',
					 // title:false,
					 // maxmin: false,
					 shade: [0.5, '#000'],
					 //配置遮罩层颜色和透明度
