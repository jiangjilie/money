	...
	<head>
			              ...
			              <fieldset>
			                  <div class="form-group">
								  <div class="col-sm-2" style="width:106px">
								    ...
								    <div class="col-sm-2">
								          <select name="company" class="form-control" id="company" style="height:36px;outline:0;margin-top: -5px;">
			              ...
			              <fieldset>
			                  <div class="form-group" >
			                      <div  style="float: right;">
									 ...
									 <a href="javascript:" id="add" title="添加员工信息" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-add" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">添加员工</span><span class="l-btn-icon icon-add">&nbsp;</span></span></a>												
									<a href="javascript:;" id="import" title="点击导入记录" onclick="inportExcel()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-import" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">导入员工</span><span class="l-btn-icon icon-import">&nbsp;</span></span></a>
		...
		<thead>
		  <tr style="background-color:#438eb9;color: #FFF;">
			...
			<th class="tr-th" style="text-align:center;padding:10px">所属公司</th>
			<th class="tr-th" style="text-align:center;padding:10px">部门</th>				
			<th class="tr-th" style="text-align:center;padding:10px">职位</th>
			<th class="tr-th" style="text-align:center;padding:10px">身份证</th>
			<th class="tr-th" style="text-align:center;padding:10px">电话</th>
			<th class="tr-th" style="text-align:center;padding:10px">银行账号</th>
			<th class="tr-th" style="text-align:center;padding:10px">入职时间</th>
		...
		<script>
			...
			$("#add").click(function(){
				...
				layer.open({
					 ...
					 closeBtn: 1,
					 anim: 2,
					 content: "user_luru.php?user="+user,
					  // btn: ["", "取消"],
		...
		$("#button").click(function(){
			 // $('#tb_user').bootstrapTable('destroy'); //动态加载表格之前，先销毁表格
				...
				var search= "dept=" + dept + "&name=" + name + "&company=" + company ;
					$.ajax({
					...
					success:function(msg){
									...
									var   data = eval("("+msg+")"); //将返回的json数据进解析，并赋给data									
										...
										function excel_select(ids) {
											window.location.href= "excel_user_select.php?id=" + ids ;
													}
						...
						function ShowDel(id) {	
							var delid= "id=" + id;
								layer.open({
								 ...
								 anim: 2,
								 shadeClose: true,
								 content: '<center><h4 style="color:red;padding:5px5px;magin:2px;">确定删除该员工吗？</br>工资表等数据会一并删除<br>请谨慎操作！</h4></center>',
								 btn: ["确定", "取消"],
								 yes: function () {
									 var delid= "id=" + id;
									 $.ajax({
									...
									var company = $("#company").val();
									 if(dept=="" &&  name=="" && company=="" ){
											layer.open({
											 ...
											 offset:'t',  //设置弹出位置
											 shadeClose: true,
											 content: 'delete_user.php?dept=' + dept + '&name=' + name + '&company=' + company ,
											 ...
											 yes: function (index, layero) {
												 var thisIndex = index;
												  ...
												  layer.confirm('<h4 style="color:red;padding:5px5px;magin:2px;">警告：真的确定清空表中所有数据吗</h4></center>', { icon: 5, title: '提示' }, function (index) {
														window.location.href= "delete_user_php.php?dept=" + dept + "&name=" + name + "&company=" + company ;										                                									                 
																																			});
																			},//layer嵌套  *****重要
														});
														}else{
			...
			<script>
				...
				function excel(){				
					...
					// window.open(url, "_blank");
					window.location.href= "excel_user.php?dept=" + dept + "&name=" + name + "&company=" + company;
								}
