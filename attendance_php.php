...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				//连接数据库
					...
					while($row = mysql_fetch_assoc($result) ){
							...
							// $cd=$row["cda"]+$row["cdb"];
							// $dk=$row["dka"]+$row["dkb"];
							// $data[]=array("id"=>$row["id"],"time"=>$row["time"],"company"=>$row["company"],"nd"=>$row["nd"],"yf"=>$row["yf"],"date"=>$row["date"],"name"=>$row["name"],"dept"=>$row["dept"],"sj"=>$sj,"sjk"=>$sjk,"bj"=>$bj,"bjk"=>$bjk,"nj"=>$row["nja"],"njk"=>$row["njb"],"cd"=>$cd,"cdk"=>$row["cdc"],"dk"=>$dk,"dkk"=>$row["dkc"],"qt"=>$row["qta"],"qtk"=>$row["qtb"],"hj"=>$row["hj"],"bz"=>$row["bz"]);
							$data[] = $row;
						}
