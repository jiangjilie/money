...
/**
    ...
    function expExcel($arr,$name){
        ...
        $objPHPExcel->getActiveSheet()
           ...
           ->setCellValue('D1', '身份证')
           ->setCellValue('E1', '电话')
           ->setCellValue('F1', '银行账户')
           ->setCellValue('G1', '入职时间')
           ->setCellValue('H1', '备注')
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
               ...
               ->setCellValue('C'.$key, $v['zw'])
               ->setCellValue('D'.$key, $v['sfz'])
               ->setCellValue('E'.$key, $v['phone'])
               ->setCellValue('F'.$key, $v['bank'])
               ->setCellValue('G'.$key, $v['rzsj'])
 ...
 $wherelist = array();//获取查询条件
 	...
 	if(!empty($_GET['company'])){
 		$wherelist[] = "company like '%{$_POST['company']}%'";
 	}
 ...
 //先获取数据
 $sql = "select * from user $where order by dept";
 $res = mysql_query($sql);
 $arr = array();
