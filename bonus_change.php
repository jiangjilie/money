...
<html>
	...
	<body>
		...
		<div id='divinput' class='cyinputdiv' style='height:460px;'>
					<form method='post' action='user_changephp.php' id='Form'>
		<?php
			...
			//连接数据库
				...
				$sql = "select * from bonus where id='$id'";//查询语句	
					...
					<div class='row'>
						...
						<div class='col-md-12'>
							...
							<div class='box box-primary'>
								<!-- /.box-header -->
								<h3><span>员工信息修改表</span></h3>
								<table>
									<tr>
																<td class='datab-1 tdtitle'>日期</td>
																<td class='datab-2'><input type='month'class='txtarea' id='date'name='setting[date]' value='{$row['date']}' style='color:black'></td>
									                           <td class='datab-1 tdtitle'>姓名</td>
									                           <td class='datab-2'><textarea class='txtarea' id='name'name='setting[name]'>{$row['name']}</textarea></td>
																<td class='datab-1 tdtitle'>部门</td>
																<td class='datab-2'>
																<select name='setting[dept]' class='form-control' id='dept' style='height:36px;outline:0;margin-top: -5px;color:black'>
																    <option>{$row['dept']}</option>			              		            
																</select>
																</td>                          
									                       </tr>
									                       <tr>
									                           <td class='datab-1 tdtitle'>行政考核</td>
									                           <td class='datab-2'><textarea class='txtarea' id='dwmc'name='setting[xz]'>{$row['xz']}</textarea></td>
									                           <td class='datab-1 tdtitle'>市场经营</td>
									                           <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[sc]'>{$row['sc']}</textarea></td>
									                           <td class='datab-1 tdtitle'>项目研发</td>
									                           <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[xm]'>{$row['xm']}</textarea></td>
									                       </tr>
									                       <tr>                        
									                         <td class='datab-1 tdtitle'>质量保障</td>
									                         <td class='datab-2'><textarea class='txtarea' id='dwmc'name='setting[zl]'>{$row['zl']}</textarea></td>
									                         <td class='datab-1 tdtitle'>软件实施</td>
									                         <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[rj]'>{$row['rj']}</textarea></td>
									                         <td class='datab-1 tdtitle'>数据实施</td>
									                         <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[sj]'>{$row['sj']}</textarea></td>
									                       </tr>
															<tr>
															  <td class='datab-1 tdtitle'>集成实施</td>
															  <td class='datab-2'><textarea class='txtarea' id='dwmc'name='setting[jc]'>{$row['jc']}</textarea></td>
															  <td class='datab-1 tdtitle'>交通津贴</td>
															  <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[jt]'>{$row['jt']}</textarea></td>
															  <td class='datab-1 tdtitle'>全员经营</td>
															  <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[qy]'>{$row['qy']}</textarea></td>
															</tr>
															<tr>
															  <td class='datab-1 tdtitle'>经理层考核</td>
															  <td class='datab-2'><textarea class='txtarea' id='dwmc'name='setting[jl]'>{$row['jl']}</textarea></td>
															  <td class='datab-1 tdtitle'>备注</td>
															  <td class='datab-2'><textarea class='txtarea' id='htmc'name='setting[bz]'>{$row['bz']}</textarea></td>
						...
						function subForm(){
							var htbh = document.getElementById('name').value;
									if(htbh==''){
									layer.alert('姓名不能为空！', {
									icon: 5,
									title: '提示',
						...
						<script>
							function add(){
								...
								// end:function(){
								// 	window.location.href="user_change.php?id=<?php echo $id; ?>"
								// }
								});
							}
						</script>
