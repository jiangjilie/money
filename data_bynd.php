...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				$sql = "select nd,company,sum(jc) as jc,sum(fd) as fd,sum(jx) as jx,sum(qx) as qx,sum(kh) as kh,sum(bt) as bt,sum(gjj) as gjj,sum(yal) as yal,sum(yil) as yil,sum(sy) as sy,sum(se) as se from salary $where group by company,nd order by nd ";//查询语句	;
