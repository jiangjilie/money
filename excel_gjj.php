...
/**
    ...
    function expExcel($arr,$name){
        ...
        $objPHPExcel->getActiveSheet()
           ...
           ->setCellValue('B1', '公司')
           ->setCellValue('C1', '部门')
           ->setCellValue('D1', '单位公积金')
           ->setCellValue('E1', '个人公积金')
           ->setCellValue('F1', '合计')
           ->setCellValue('G1', '备注')
           ->setCellValue('H1', '日期')           
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
               ...
               ->setCellValue('B'.$key, $v['company'])
               ->setCellValue('C'.$key, $v['dept'])
               ->setCellValue('D'.$key, $v['gjja'])
               ->setCellValue('E'.$key, $v['gjjb'])
               ->setCellValue('F'.$key, $v['hj'])
               ->setCellValue('G'.$key, $v['bz'])
               ->setCellValue('H'.$key, $v['date']);
 ...
 //先获取数据
 $sql = "select * from gjj $where order by date";
 $res = mysql_query($sql);
 $arr = array();
 //把$res=>$arr,把结果集内容转移到一个数组中
 while ($row = mysql_fetch_assoc($res)){
  $hj=$row["gjja"]+$row["gjjb"];
  //将查询结构集重新数组化
  $arr[]=array("date"=>$row["date"],"name"=>$row["name"],"dept"=>$row["dept"],"company"=>$row["company"],"gjja"=>$row["gjja"],"gjjb"=>$row["gjjb"],,"hj"=>"$hj","bz"=>$row["bz"]);
  // $data[] = $row;
 }
