...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				//连接数据库
					...
					while($row = mysql_fetch_assoc($result) ){
							...
							$yf=intval($row['yf']);	
							//将查询结构集重新数组化
							$data[]=array("id"=>$row["id"],"time"=>$row["time"],"company"=>$row["company"],"dept"=>$row["dept"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"jc"=>$row["jc"],"fd"=>$row["fd"],"jx"=>$row["jx"],"qx"=>$row["qx"],"kh"=>$row["kh"],"bt"=>$row["bt"],"gjj"=>$row["gjj"],"yal"=>$row["yal"],"yil"=>$row["yil"],"sy"=>$row["sy"],"se"=>$row["se"],"yfhj"=>$yfhj,"sfhj"=>$sfhj);
							// $data[] = $row;
						}
