...
/**
    ...
    function expExcel($arr,$name,$data){
        ...
        $objPHPExcel->getActiveSheet()
           ->setCellValue('A1', '日期')
           ->setCellValue('B1', '姓名')
           ->setCellValue('C1', '基础工资')
           ->setCellValue('D1', '浮动工资')
           ->setCellValue('E1', '月度绩效')
           ->setCellValue('F1', '缺薪')
           ->setCellValue('G1', '考核')
           ->setCellValue('H1', '补贴')
           ->setCellValue('I1', '应发合计')
           ->setCellValue('J1', '公积金')
           ->setCellValue('K1', '养老')
           ->setCellValue('L1', '医疗')
           ->setCellValue('M1', '失业')
           ->setCellValue('N1', '个税')
           ->setCellValue('O1', '实发合计')
           ->setCellValue('P1', '备注')
           ->setCellValue('Q1', '公司')
           ->setCellValue('R1', '部门')
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
               ...
               ->setCellValue('A'.$key, $v['date'])
               ->setCellValue('B'.$key, $v['name'])
               ->setCellValue('C'.$key, $v['jc'])
               ->setCellValue('D'.$key, $v['fd'])
               ->setCellValue('E'.$key, $v['jx'])
               ->setCellValue('F'.$key, $v['qx'])
               ->setCellValue('G'.$key, $v['kh'])
               ->setCellValue('H'.$key, $v['bt'])
               ->setCellValue('I'.$key, $v['yfhj'])              
               ->setCellValue('J'.$key, $v['gjj'])              
               ->setCellValue('K'.$key, $v['yal'])              
               ->setCellValue('L'.$key, $v['yil'])              
               ->setCellValue('M'.$key, $v['sy'])              
               ->setCellValue('N'.$key, $v['se'])              
               ->setCellValue('O'.$key, $v['sfhj'])                          
               ->setCellValue('P'.$key, $v['bz'])
               ->setCellValue('Q'.$key, $v['company'])
		...
		foreach($data as $v){
		    ...
		    $objPHPExcel->getActiveSheet()
		        ...
		        //Excel的第A列，name是你查出数组的键值字段，下面以此类推
		        ->setCellValue('A'.$key, $v['name'].$v['nd'].'年度合计：')
		        ->setCellValue('B'.$key, '')
		      ->setCellValue('C'.$key, $v['jc'])
		      ->setCellValue('D'.$key, $v['fd'])
		      ->setCellValue('E'.$key, $v['jx'])
 ...
 //先获取数据
 $sql = "select * from salary $where order by date";
 $res = mysql_query($sql);
 $arr = array();
 ...
 while ($row = mysql_fetch_assoc($res)){
  ...
  $yf=intval($row['yf']);	
  //将查询结构集重新数组化
  $arr[]=array("id"=>$row["id"],"time"=>$row["time"],"date"=>$row["date"],"company"=>$row["company"],"dept"=>$row["dept"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"jc"=>$row["jc"],"fd"=>$row["fd"],"jx"=>$row["jx"],"qx"=>$row["qx"],"kh"=>$row["kh"],"bt"=>$row["bt"],"gjj"=>$row["gjj"],"yal"=>$row["yal"],"yil"=>$row["yil"],"sy"=>$row["sy"],"se"=>$row["se"],"yfhj"=>$yfhj,"sfhj"=>$sfhj);
  // $data[] = $row;
 }
 
$sql = "select nd,company,name,dept,sum(jc) as jc,sum(fd) as fd,sum(jx) as jx,sum(qx) as qx,sum(kh) as kh,sum(bt) as bt,sum(gjj) as gjj,sum(yal) as yal,sum(yil) as yil,sum(sy) as sy,sum(se) as se from salary $where group by company,dept,name,nd order by nd ";//查询语句	;
 $result = mysql_query($sql);
  while($row = mysql_fetch_assoc($result)){
	...
	$yfhj=number_format($yfhj,2);
	$sfhj=number_format($sfhj,2);
	$data[]=array("date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"jc"=>$row["jc"],"fd"=>$row["fd"],"jx"=>$row["jx"],"qx"=>$row["qx"],"kh"=>$row["kh"],"bt"=>$row["bt"],"gjj"=>$row["gjj"],"yal"=>$row["yal"],"yil"=>$row["yil"],"sy"=>$row["sy"],"se"=>$row["se"],"yfhj"=>$yfhj,"sfhj"=>$sfhj);						
							}
 //excel表格名
