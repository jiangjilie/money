...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				$sql = "select date,dept,sum(jc) as jc,sum(fd) as fd,sum(jx) as jx,sum(qx) as qx,sum(kh) as kh,sum(bt) as bt,sum(gjj) as gjj,sum(yal) as yal,sum(yil) as yil,sum(sy) as sy,sum(se) as se from salary $where group by date order by date ";//查询语句	;
					...
					while($row = mysql_fetch_assoc($result) ){
							...
							$yf=intval($row['yf']);	
							//将查询结构集重新数组化
							$data[]=array("id"=>$row["id"],"time"=>$row["time"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"jc"=>$row["jc"],"fd"=>$row["fd"],"jx"=>$row["jx"],"qx"=>$row["qx"],"kh"=>$row["kh"],"bt"=>$row["bt"],"gjj"=>$row["gjj"],"yal"=>$row["yal"],"yil"=>$row["yil"],"sy"=>$row["sy"],"se"=>$row["se"],"yfhj"=>$yfhj,"sfhj"=>$sfhj);
							// $data[] = $row;
						}
