...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
   ...
   //var_dump($strs);exit;
  //---------------------------------------------------重要------------------------------------ 
   $sql="select *  from salary where date='$date' and name='{$strs[1]}' ";
   $result = mysql_query($sql);
   if (!mysql_num_rows($result)){      //判断是否是第一次添加，是则插入数据------------------------------------------
	$sql = "insert into salary (company,date,nd,yf,name,jc,fd) values ('{$company}','{$date}','{$nd}','{$yf}','{$strs[1]}','{$strs[2]}','{$strs[25]}')";
      }else{   //否则说明有记录，那就在原有数据基础上更新------------------------------------------------					
   	$sql = "update salary set name='{$strs[1]}',jc='{$strs[2]}',fd='{$strs[25]}' where date='$date'and company='$company' and name='{$strs[1]}'";													
	}
   if(!mysql_query($sql)){ 
	echo "
	<script>layer.alert('导入失败！', {
       icon: 2,
       title: '提示'
     });</script>"; exit(); 
	 }else{
	echo "
   <script>layer.alert('导入成功！', {
       icon: 1,
       title: '提示'
     });</script>"; 
	 }
