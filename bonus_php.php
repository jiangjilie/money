...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				//连接数据库
					...
					$sql = "select * from bonus $where ";
					$result = mysql_query($sql,$link);					
					while($row = mysql_fetch_assoc($result) ){
							$jx=$row['xz']+$row['sc']+$row['xm']+$row['zl']+$row['rj']+$row['sj']+$row['jc']+$row['jt'];
							$jj=$jx+$row['qy']+$row['jl'];
							$jx=number_format($jx,2);
							$jj=number_format($jj,2);
							$yf=intval($row['yf']);	
							//将查询结构集重新数组化
							$data[]=array("id"=>$row["id"],"time"=>$row["time"],"company"=>$row["company"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"dept"=>$row["dept"],"xz"=>$row["xz"],"sc"=>$row["sc"],"xm"=>$row["xm"],"zl"=>$row["zl"],"rj"=>$row["rj"],"sj"=>$row["sj"],"jc"=>$row["jc"],"jt"=>$row["jt"],"qy"=>$row["qy"],"jl"=>$row["jl"],"jj"=>$jj,"jx"=>$jx);
							// $data[] = $row;
						}
