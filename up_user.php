...
<html>
 ...
 <div id="tb" style="padding:5px;height:auto;background-color:#438eb9;color: white;" class="datagrid-toolbar">
 	<div id="divtoolbar" style="margin-bottom:5px">
		<form action="up_user.php" method="post" enctype="multipart/form-data" id='Form'>
	...
	<script>
		function subForm(){
			var date = document.getElementById("company").value;
			if(date==''){
			layer.alert('请选择所属公司！', {
			icon: 5,
			title: '提示',
		...
		layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="padding:5px5px;magin:2px;">确定将数据导入到<span style="color:red;">员工表</span>吗</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
			                    $("#Form").submit();
			                 },
...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
   ...
   $sql = "insert into user (name,dept,zw,sfz,phone,bank,rzsj,bz,company) values ('{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[6]}','{$strs[7]}','$company')";
...
<script>
 ...
 title: '提示'
  ...
  $sql = "select * from dept where name='{$strs[1]}' and company='$company' ";//查询语句
  $result=mysql_query($sql);
  $row = mysql_fetch_assoc($result);
  if(!$row){
  $insert = "insert into dept (name,company) values('{$strs[1]}','$company')";   //判断部门表中该公司有无所选部门，若没有则新增
  }
  mysql_query($insert);
   }
}
