    ...
    <section class="content" style="height:100%;">
        <div class="row" style="height:100%;">
			    ...
			    <div class="box box-primary" style="height: 99%;">
			        <div class="box-header with-border">
			            ...
			            <div class="box-tools pull-right" style="top:20%">		           
			                <a title="添加公司信息" id="btnAddCom" class="green" onclick="AddCom();" href="javascript://"><i class="fa fa-plus"></i></a>
			            </div>
						</form>
            ...
            <div class="col-md-4" style="height:100%;">
                ...
                <div class="box box-primary" style="height: 99%;">
                    <div class="box-header with-border">
                        <h3 class="box-title" ><?php echo $_GET["company"]; ?> -> 部门</h3>
                                ...
                                <div class="col-md-4">
                                    <div id="EepartMentTree" class="treeview" style="width: 112%;">
										<!-- <ul class="list-group" id="suggest_ul"> -->
										 ...
										 $sql = "select * from dept where company='$company' ";//查询语句	;
										 $result = mysql_query($sql,$link);
										 echo"<ul  class='list-group' id='suggest_ul'>";
										 while($row = mysql_fetch_assoc($result)){
										 	echo "<li id='{$row["name"]}'style='background-color:;padding:2px;list-style:none' class='cd'><a style='text-decoration:none;'href='dept?dept={$row["name"]}&company={$row['company']}'>{$row["name"]}</a>
            ...
            <div class="col-md-3" style="height:100%;">
                    <div class="box box-primary" style="height: 99%;">
                        <div class="box-header with-border">
                            <h3 class="box-title"><?php echo $_GET["company"]; ?> -> <?php echo $_GET["dept"]; ?> -> 员工</h3>
                                    ...
                                    <div class="col-md-3">
                                        <div id="UserTree" class="treeview"style="width: 110%;">
											<!-- <ul class="list-group" id="user_ul"> -->
											 ...
											 mysql_select_db(DBNAME,$link);//选择数据库
											 mysql_query("set names 'utf8'");//选择字符集
											 $sql = "select * from user where dept='$dept' and  company='$company' ";//查询语句	;
											 $result = mysql_query($sql,$link);
...
<script>
	...
	layer.open({
						 ...
						 anim: 2,
						 shadeClose: true,
						 content: 'dept_luru?company=<?php echo $_GET["company"]; ?> ',
						 // btn: ["添加", "取消"],
						 end: function () {
	...
	function AddUser(){
		layer.open({
						 ...
						 anim: 2,
						 shadeClose: true,
						 content: 'user_luru?dept=<?php echo $_GET["dept"]; ?>&company=<?php echo $_GET["company"]; ?> ',
						 // btn: ["添加", "取消"],
						 end: function () {
	...
	function EditCom(id){
		layer.open({
			 ...
			 closeBtn: 1,
			 anim: 2,
			 content: "company_change.php?id=" + id,
			  // btn: ["", "取消"],
	...
	function DelCom(id) {
		var delid= "id=" + id;
			layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="color:red;padding:5px5px;magin:2px;">确定删除该公司吗？<br>删除会将该公司所有部门所有员工全部删除<br>包括工资表等数据<br>请谨慎操作！</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
				 var delid= "id=" + id;
				 $.ajax({
	...
	function DelEepartMent(id) {
		var delid= "id=" + id;
			layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="color:red;padding:5px5px;magin:2px;">确定删除该部门吗？<br>删除会将该部门员工全部删除<br>包括工资表等数据<br>请谨慎操作！</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
				 var delid= "id=" + id;
				 $.ajax({
		...
		success: function (msg) {
			...
			for(var i=0;i<data.length;i++){
				...
				// contents=contents+"<option>"+keywords+"</option>";	
				// contents=contents+"<option value='"+keywords+"'>"+keywords+"</option>";	
				$("#company_dept").append("<option value='"+keywords+"'>"+keywords+"</option>");
			}
			// $("#dept").html(contents);
		 },
		});
