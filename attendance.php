	...
	<head>
			              ...
			              <fieldset>
			                  <div class="form-group" >
			                      <div  style="float: right;">
									 ...
									 <!-- <a href="javascript:" id="add" title="添加考勤信息" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-add" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">添加考勤</span><span class="l-btn-icon icon-add">&nbsp;</span></span></a>												 -->
									<a href="javascript:;" id="import" title="点击导入记录" onclick="inportExcel()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-import" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">导入考勤</span><span class="l-btn-icon icon-import">&nbsp;</span></span></a>
		...
		<thead>
		  <tr style="background-color:#438eb9;color: #FFF;">
			...
			<th class="tr-th" style="text-align:center;padding:10px">扣浮动事假</th>
			<th class="tr-th" style="text-align:center;padding:10px">全扣事假</th>
			<th class="tr-th" style="text-align:center;padding:10px">扣浮动病假</th>
			<th class="tr-th" style="text-align:center;padding:10px">全扣病假</th>
			<th class="tr-th" style="text-align:center;padding:10px">扣浮动年假</th>
			<th class="tr-th" style="text-align:center;padding:10px">上午迟到</th>
			<th class="tr-th" style="text-align:center;padding:10px">下午迟到</th>
			<th class="tr-th" style="text-align:center;padding:10px">上午未打卡</th>
			<th class="tr-th" style="text-align:center;padding:10px">下午未打卡</th>
			<th class="tr-th" style="text-align:center;padding:10px">扣浮动其他假</th>
			<th class="tr-th" style="text-align:center;padding:10px">合计扣款</th>
		...
		<script>
			...
			$("#add").click(function(){
				...
				layer.open({
					 ...
					 closeBtn: 1,
					 anim: 2,
					 content: "attendance_luru.php?user="+user,
					  // btn: ["", "取消"],
		...
		$("#button").click(function(){
									...
									var   data = eval("("+msg+")"); //将返回的json数据进解析，并赋给data									
										...
										function deleteMs(ids) {
											...
											title: '提示',
											 ...
											 yes:function(){
												$.ajax({
												    url : 'attendance_delete_batch.php',
												    // data : "id=" + ids,
												    data : "id=" + ids,
												    type : "post",
										...
										function excel_select(ids) {
											window.location.href= "excel_attendance_select.php?id=" + ids ;
													}
				...
				//修改
					function ShowEdit(id){
						...
						layer.open({
							 ...
							 closeBtn: 1,
							 anim: 2,
							 content: "attendance_change.php?id=" + id,
							  // btn: ["", "取消"],
									...
									var nd = $("#yf").val();
									 if(dept=="" &&  name=="" && nd==""&& yf=="" ){
											layer.open({
											 ...
											 offset:'t',  //设置弹出位置
											 shadeClose: true,
											 content: 'delete_attendance.php?dept=' + dept + '&name=' + name + '&nd=' + nd+ '&yf=' + yf ,
											 ...
											 yes: function (index, layero) {
												 var thisIndex = index;
												  ...
												  layer.confirm('<h4 style="color:red;padding:5px5px;magin:2px;">警告：真的确定清空表中所有数据吗</h4></center>', { icon: 5, title: '提示' }, function (index) {
														window.location.href= "delete_attendance_php.php?dept=" + dept + "&name=" + name + "&phone=" + phone ;										                                									                 
																																			});
																			},//layer嵌套  *****重要
														});
														}else{
			...
			<script>
				...
				function excel(){				
					...
					// window.open(url, "_blank");
					window.location.href= "excel_attendance.php?dept=" + dept + "&name=" + name + "&nd=" + nd+ "&yf=" + yf+"&company="+company;
								}
