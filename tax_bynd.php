...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				//连接数据库
					...
					$sql = "select nd,company,name,sum(se) as se from tax $where group by nd,company,name";
					$result = mysql_query($sql,$link);					
					while($row = mysql_fetch_assoc($result) ){
							// $hj=$row["gjj"]+$row["ylj"]+$row["yl"]+$row["sy"];
							// //将查询结构集重新数组化
							// $data[]=array("id"=>$row["id"],"time"=>$row["time"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"dept"=>$row["dept"],"gjj"=>$row["gjj"],"ylj"=>$row["ylj"],"yl"=>$row["yl"],"sy"=>$row["sy"],"hj"=>$hj);
							$data[] = $row;
						}
