...
<html>
 ...
 <div id="tb" style="padding:5px;height:auto;background-color:#438eb9;color: white;" class="datagrid-toolbar">
 	<div id="divtoolbar" style="margin-bottom:5px">
		<form action="up_bonus.php" method="post" enctype="multipart/form-data" id='Form'>
		...
		<img title=''  style="width: 1200px;"src='images/bonus.png'>	
		</div>
		<div style="margin: 8px;padding:2px;border:2px;background-color: ;">
	...
	<script>
		...
		layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="padding:5px5px;magin:2px;">确定将数据导入到<span style="color:red;">奖金表</span>吗</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
			                    $("#Form").submit();
			                 },
...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
   ...
   $sql = "insert into bonus (company,date,nd,yf,name,dept,jx,xz,sc,xm,zl,rj,sj,jc,jt,qy,jl,jj) values ('{$company}','{$date}','{$nd}','{$yf}','{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[6]}','{$strs[7]}','{$strs[8]}','{$strs[9]}','{$strs[10]}','{$strs[11]}','{$strs[12]}','{$strs[13]}')";
...
<script>
 ...
 title: '提示'
   ...
   $sql = "update salary set dept='{$strs[1]}',jx='{$strs[13]}' where date='$date'and name='{$strs[0]}'";
   mysql_query($sql);
   }
}
