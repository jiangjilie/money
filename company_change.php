...
<html>
	...
	<body>
		...
		<div id='divinput' class='cyinputdiv' style='height:460px;'>
					<form method='post' action='company_changephp.php' id='Form'>
		<?php
					...
					<div class='row'>
						...
						<div class='col-md-12'>
							...
							<div class='box box-primary'>
								<!-- /.box-header -->
								<h3><span>公司信息修改表</span></h3>
								<table>

								   <tr>
									   <td class='datab-1 tdtitle'>公司名称</td>
									   <td class='datab-2'><textarea class='txtarea' id='company'name='company'>{$row['name']}</textarea></td>
									   <textarea class='txtarea' id=''name='ycompany' style='display:none'>{$row['name']}</textarea>
									   <td class='datab-1 tdtitle'>备注</td>
									   <td class='datab-2'><textarea class='txtarea' id='bz'name='bz'>{$row['bz']}</textarea></td>
						...
						<script>
						   ...
						   $('#qlc_zdz1').on('change', function(){
						   	...
						   	document.getElementById("company").value =this.value;
						   });
						...
						function subForm(){
							var htbh = document.getElementById('company').value;
									if(htbh==''){
									layer.alert('部门不能为空！', {
									icon: 5,
									title: '提示',
						...
						<script>
							function add(){
								...
								layer.open({
								 ...
								 anim: 2,
								 shadeClose: true,
								 content: 'bill_company.php?htbh='+htbh+'&xmbh='+xmbh+'&xmmc='+xmmc+'&dwmc='+dwmc+'&htmc='+htmc,
								// end:function(){
								// 	window.location.href="company_change.php?id=<?php echo $id; ?>"
								// }
								});
							}
						</script>
