...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				//连接数据库
					...
					$sql = "select * from subsidy $where ";
					$result = mysql_query($sql,$link);					
					while($row = mysql_fetch_assoc($result) ){
							// $hj=$row["tx"]+$row["jt"]+$row["wc"]+$row["zm"]+$row["jb"]+$row["sr"];
							// //将查询结构集重新数组化
							// $data[]=array("id"=>$row["id"],"time"=>$row["time"],"company"=>$row["company"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"dept"=>$row["dept"],"tx"=>$row["tx"],"jt"=>$row["jt"],"wc"=>$row["wc"],"zm"=>$row["zm"],"jb"=>$row["jb"],"sr"=>$row["sr"],"hj"=>$hj);
							$data[] = $row;
						}
