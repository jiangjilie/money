...
<html>
 ...
 <div id="tb" style="padding:5px;height:auto;background-color:#438eb9;color: white;" class="datagrid-toolbar">
 	<div id="divtoolbar" style="margin-bottom:5px">
		<form action="up_subsidy.php" method="post" enctype="multipart/form-data" id='Form'>
		...
		<img title=''  style="width: 1200px;"src='images/subsidy.png'>	
		</div>
		<div style="margin: 8px;padding:2px;border:2px;background-color: ;">
	...
	<script>
		...
		layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="padding:5px5px;magin:2px;">确定将数据导入到<span style="color:red;">补贴表</span>吗</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
			                    $("#Form").submit();
			                 },
...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
   ...
   for($j=4;$j<=$highestRow;$j++)                        //从第二行开始读取数据
   ...
   //var_dump($strs);exit;
   $hj=$strs[3]+$strs[4]+$strs[5]+$strs[6]+$strs[7]+$strs[8]+$strs[9];
   $sql = "insert into subsidy (company,date,nd,yf,dept,name,tx,jt,wc,zm,jb,sr,qt,bz,hj) values ('{$company}','{$date}','{$nd}','{$yf}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[6]}','{$strs[7]}','{$strs[8]}','{$strs[9]}','{$strs[10]}','$hj')";
...
<script>
 ...
 $sql = "update salary set dept='{$strs[1]}',bt='$hj' where date='$date' and name='{$strs[2]}' and company='$company' ";
 mysql_query($sql); 
   }
}
