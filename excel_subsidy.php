...
/**
    ...
    function expExcel($arr,$name){
        ...
        $objPHPExcel->getActiveSheet()
           ...
           ->setCellValue('E1', '交通费')
           ->setCellValue('F1', '误餐费')
           ->setCellValue('G1', '周末工作餐费')
           ->setCellValue('H1', '节假日加班费')
           ->setCellValue('I1', '生日费')
           ->setCellValue('J1', '合计')         
           ->setCellValue('K1', '备注')         
           ->setCellValue('L1', '日期')         
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
               ...
               ->setCellValue('D'.$key, $v['tx'])
               ->setCellValue('E'.$key, $v['jt'])
               ->setCellValue('F'.$key, $v['wc'])
               ->setCellValue('G'.$key, $v['zm'])
               ->setCellValue('H'.$key, $v['jb'])
               ->setCellValue('I'.$key, $v['sr'])           
               ->setCellValue('J'.$key, $v['hj'])           
               ->setCellValue('K'.$key, $v['bz'])           
               ->setCellValue('L'.$key, $v['date']);           
 ...
 //先获取数据
 $sql = "select * from subsidy $where order by date";
 $res = mysql_query($sql);
 $arr = array();
 //把$res=>$arr,把结果集内容转移到一个数组中
 while ($row = mysql_fetch_assoc($res)){
  $hj=$row["tx"]+$row["jt"]+$row["wc"]+$row["zm"]+$row["jb"]+$row["sr"];
  //将查询结构集重新数组化
  $arr[]=array("id"=>$row["id"],"time"=>$row["time"],"date"=>$row["date"],"nd"=>$row["nd"],"yf"=>$yf,"name"=>$row["name"],"dept"=>$row["dept"],"tx"=>$row["tx"],"jt"=>$row["jt"],"wc"=>$row["wc"],"zm"=>$row["zm"],"jb"=>$row["jb"],"sr"=>$row["sr"],"hj"=>$hj);
  // $data[] = $row;
 }
